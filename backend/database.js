...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS repairs (
    ...
    diagnostic_duration_hours INTEGER DEFAULT 48,
    diagnostic_completed BOOLEAN DEFAULT 0,
    progress_started_at DATETIME,
    progress_duration_hours INTEGER DEFAULT 72,
...
// تشغيل التحديثات (migrations)
try {
  const MigrationRunner = require('./migrations/runner');
  const runner = new MigrationRunner(db);
  runner.run();
} catch (err) {
  console.log('ℹ️ Migration runner not available or error:', err.message);
}
