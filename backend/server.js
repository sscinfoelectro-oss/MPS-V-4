...
// ============================================
// 🚗 VEHICLES (par utilisateur connecté)
// ============================================
...
app.post('/api/vehicles', authenticateToken, (req, res) => {
  const { name, year, plate_number } = req.body;
  if (!name || !year) {
    return res.status(400).json({ error: 'الاسم والسنة مطلوبان' });
  }
  try {
    const result = db.prepare(`
      INSERT INTO vehicles (name, year, plate_number, user_id, status, created_at)
      VALUES (?, ?, ?, ?, 'ok', datetime('now'))
    `).run(name, year, plate_number || null, req.user.id);
    res.json({ success: true, id: result.lastInsertRowid, message: '🚗 Véhicule ajouté avec succès!' });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

// Modifier un véhicule (vérification de propriété)
app.put('/api/vehicles/:id', authenticateToken, (req, res) => {
  const { id } = req.params;
  const { name, year, plate_number } = req.body;
  try {
    const vehicle = db.prepare('SELECT user_id FROM vehicles WHERE id = ?').get(id);
    if (!vehicle) return res.status(404).json({ error: 'Véhicule introuvable' });
    if (vehicle.user_id !== req.user.id) return res.status(403).json({ error: 'غير مصرح' });
    
    db.prepare(`
      UPDATE vehicles SET name = ?, year = ?, plate_number = ? WHERE id = ?
    `).run(name, year, plate_number || null, id);
...
// Supprimer un véhicule (vérification de propriété)
app.delete('/api/vehicles/:id', authenticateToken, (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, message: '🗑️ Véhicule supprimé avec succès!' });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
