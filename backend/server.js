...
// ============================================
// 🚗 MODULAR VEHICLE ROUTES (Layers 1, 2, 3)
// ============================================
...
app.post('/api/auth/register', (req, res) => {
  ...
  try {
    ...
    const vehicleType = carType || '';
      const sqlQuery = 'INSERT INTO users (phone, password, vehicle_type, role) VALUES (?, ?, ?, ?)';
      logTime(sqlQuery);
      const result = db.prepare(sqlQuery).run(phone, hashedPassword, vehicleType, 'user');
...
app.delete('/api/admin/garages/:id', authenticateToken, isAdmin, (req, res) => {
  try {
    ...
    // حذف السجلات المرتبطة أولاً لتجنب مشكلة FOREIGN KEY
    db.exec('BEGIN TRANSACTION');
    db.prepare('DELETE FROM repair_updates WHERE repair_id IN (SELECT id FROM repairs WHERE garage_id = ?)').run(id);
    db.prepare('DELETE FROM repairs WHERE garage_id = ?').run(id);
    db.prepare('DELETE FROM appointments WHERE garage_id = ?').run(id);
...
// ✅ واجهة الموافقة على الخدمة (تحويلها إلى معتمدة)
app.post('/api/services/external/:id/verify', authenticateToken, isAdmin, (req, res) => {
  const { id } = req.params;
  try {
    db.prepare('UPDATE services SET verified = 1 WHERE id = ?').run(id);
    res.json({ success: true, message: '✅ تم اعتماد الخدمة بنجاح' });
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
});
...
app.get('/api/admin/stats', authenticateToken, isAdmin, (req, res) => {
  ...
  const activeAppointments = db.prepare("SELECT COUNT(*) as count FROM appointments WHERE status NOT IN ('completed', 'cancelled')").get();
  const totalGarages = db.prepare('SELECT COUNT(*) as count FROM garages').get();
  ...
  res.json({
    ...
    activeAppointments: activeAppointments ? activeAppointments.count : 0,
    totalGarages: totalGarages ? totalGarages.count : 0,
    totalServices: totalServices ? totalServices.count : 0
  });
});
...
app.get('/api/admin/appointments', authenticateToken, isAdmin, (req, res) => {
  try {
    const appointments = db.prepare(`
      SELECT a.*, g.name as garage_name, u.phone as user_phone, r.completed_at
      FROM appointments a
      JOIN garages g ON a.garage_id = g.id
...
// 1. Start diagnostic timer
app.post('/api/admin/repairs/:id/start-diagnostic', authenticateToken, isAdmin, (req, res) => {
  const { id } = req.params;
  const { duration_hours = 48 } = req.body;

  try {
    db.prepare(`
      UPDATE repairs 
      SET diagnostic_started_at = datetime('now'), 
          diagnostic_duration_hours = ?,
          diagnostic_completed = 0,
          status = 'diagnosing'
      WHERE id = ?
    `).run(duration_hours, id);

    db.prepare(`
      INSERT INTO repair_updates (repair_id, status, message, updated_by)
      VALUES (?, 'diagnosing', '🔍 Diagnostic commencé (timer: ${duration_hours}h)', 'garage')
    `).run(id);

    res.json({ success: true, message: '✅ Diagnostic commencé' });
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
});
...
app.post('/api/admin/repairs/:id/start-progress', authenticateToken, isAdmin, (req, res) => {
  const { id } = req.params;
  const { duration_hours = 72 } = req.body;

  try {
    db.prepare(`
      UPDATE repairs 
      SET progress_started_at = datetime('now'), 
          progress_duration_hours = ?,
    ...
    db.prepare(`
      INSERT INTO repair_updates (repair_id, status, message, updated_by)
      VALUES (?, 'in_progress', '🔧 Travaux commencés (timer: ${duration_hours}h)', 'garage')
    `).run(id);

    // Update associated appointment
    const repair = db.prepare('SELECT appointment_id FROM repairs WHERE id = ?').get(id);
    if (repair && repair.appointment_id) {
      db.prepare('UPDATE appointments SET status = ? WHERE id = ?').run('in_progress', repair.appointment_id);
    }

    res.json({ success: true, message: '✅ Travaux commencés' });
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
});
...
// 3. Reset timer
app.post('/api/admin/repairs/:id/reset-timer', authenticateToken, isAdmin, (req, res) => {
  const { id } = req.params;
  const { timer_type } = req.body; // 'diagnostic' or 'progress'

  try {
    if (timer_type === 'diagnostic') {
      db.prepare(`UPDATE repairs SET diagnostic_started_at = datetime('now'), diagnostic_completed = 0 WHERE id = ?`).run(id);
    } else {
      db.prepare(`UPDATE repairs SET progress_started_at = datetime('now'), progress_completed = 0 WHERE id = ?`).run(id);
    }
    
    res.json({ success: true, message: '✅ Timer réinitialisé' });
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
});

// 4. Mark timer as complete
app.post('/api/admin/repairs/:id/complete-timer', authenticateToken, isAdmin, (req, res) => {
  const { id } = req.params;
  const { timer_type } = req.body; // 'diagnostic' or 'progress'

  try {
    if (timer_type === 'diagnostic') {
      db.prepare(`UPDATE repairs SET diagnostic_completed = 1 WHERE id = ?`).run(id);
    } else {
      db.prepare(`UPDATE repairs SET progress_completed = 1 WHERE id = ?`).run(id);
    }
    
    res.json({ success: true, message: '✅ Timer marqué comme terminé' });
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
});
...
app.get('/api/admin/users', authenticateToken, isAdmin, (req, res) => {
  const sqlUsers = 'SELECT u.id, u.phone, u.password, u.vehicle_type, u.role, u.created_at, (SELECT COUNT(*) FROM vehicles WHERE user_id = u.id) as vehicles_count FROM users u';
  logTime(sqlUsers);
