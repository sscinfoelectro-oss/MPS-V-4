      ...
      <nav className="nav-links">
        {userData ? (
          <>
            ...
            <Link to="/my-vehicles" style={location.pathname === '/my-vehicles' ? activeStyle : {}}>Mes Véhicules</Link>
    ...
    <div className="page-shell">
      ...
      <section className="hero-section" style={{ backgroundImage: `url(${heroBg})` }}>
        ...
        <div className="hero-copy">
          ...
          <p className="hero-description">
            ...
            {userData && ` - Connecté en tant que ${userData.phone}`}
          </p>
            <div className="hero-actions" style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap', alignItems: 'center' }}>
              <Link to="/garages">
      ...
      <section className="about-section">
        <div className="about-card" style={{textAlign: 'center'}}>
          ...
          <div className="features-grid" style={{display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '20px', marginTop: '30px'}}>
...
function RepairsPage() {
  ...
  const loadUpdates = async (repairId) => {
    try {
      ...
      const data = await res.json()
      setRepairUpdates(prev => ({ ...prev, [repairId]: data.updates || [] }))
    } catch (err) {
      setRepairUpdates(prev => ({ ...prev, [repairId]: [] }))
    }
  }
            ...
            <div style={{display: 'grid', gap: '12px'}}>
              ...
              {repairs.map(repair => (
                <div key={repair.id}>
                  ...
                  {/* Timeline de la réparation sélectionnée - sous l'élément lui-même */}
                  {selectedRepair?.id === repair.id && (
                    <div style={{
                      ...
                      borderRight: '2px solid #E31837',
                      borderBottom: '2px solid #E31837',
                      marginTop: '0',
                      marginBottom: '10px'
                    }}>
                      <h4 style={{color: 'white', marginBottom: '12px', fontSize: '0.95rem'}}>
                      ...
                      {selectedRepair.diagnosis_notes && (
                        <div style={{
                          ...
                          <strong style={{color: '#3b82f6', fontSize: '0.8rem'}}>Notes de diagnostic :</strong>
                          <p style={{color: '#e2e8f0', margin: '5px 0 0', fontSize: '0.85rem'}}>{selectedRepair.diagnosis_notes}</p>
                        </div>
                      )}

                      {/* 🕐 Timer display */}
                      {selectedRepair.status === 'diagnosing' && selectedRepair.diagnostic_started_at && (
                        <div style={{
                          marginBottom: '12px',
                          padding: '10px',
                          borderRadius: '8px',
                          ...
                          <span style={{color: '#94a3b8', fontSize: '0.8rem'}}>🔍 Diagnostic:</span>
                          <TimerDisplay 
                            startTime={selectedRepair.diagnostic_started_at} 
                            maxHours={selectedRepair.diagnostic_duration_hours || 48}
                            onComplete={() => alert('🕒 Le délai de diagnostic est écoulé.')}
                          />
                        </div>
                      )}

                      {selectedRepair.status === 'in_progress' && selectedRepair.progress_started_at && (
                        <div style={{
                          marginBottom: '12px',
                          padding: '10px',
                          borderRadius: '8px',
                          ...
                          <span style={{color: '#94a3b8', fontSize: '0.8rem'}}>🔧 Travaux:</span>
                          <TimerDisplay 
                            startTime={selectedRepair.progress_started_at} 
                            maxHours={selectedRepair.progress_duration_hours || 72}
                            onComplete={() => alert('🔧 Délai de réparation écoulé.')}
                          />
                        </div>
                      )}
                      ...
                      <div style={{position: 'relative'}}>
                        {(!repairUpdates[selectedRepair.id] || repairUpdates[selectedRepair.id].length === 0) ? (
                          <p style={{color: '#94a3b8', textAlign: 'center', padding: '10px', fontSize: '0.8rem'}}>
                            Aucune mise à jour pour le moment.
                          </p>
                        ) : (
                          repairUpdates[selectedRepair.id].map((update, index) => (
                            <div key={update.id} style={{
                              ...
                              {index < repairUpdates[selectedRepair.id].length - 1 && (
                                <div style={{
                                  position: 'absolute',
                                  left: '10px',
                                  top: '20px',
                              ...
                              <div style={{
                                ...
                                <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                                  <strong style={{color: getStatusColor(update.status), fontSize: '0.8rem'}}>
                                ...
                                {update.message && (
                                  <p style={{color: '#cbd5e1', margin: '4px 0 0', fontSize: '0.85rem'}}>
                                    {update.message}
                                  </p>
                                )}
                              </div>
          ...
          <div className="about-card">
            ...
            <div style={{marginTop: '25px', padding: '15px', background: 'rgba(227,24,55,0.1)', borderRadius: '10px', border: '1px solid rgba(227,24,55,0.2)'}}>
              ...
              <ul style={{color: '#cbd5e1', lineHeight: '2', listStyle: 'none', padding: 0}}>
                ...
                <li>✅ <strong>Suivi des réparations</strong> - Suivez l'avancement de vos réparations</li>
...
function AdminDashboard() {
  ...
  // ✅ مسح الرسائل تلقائياً بعد 5 ثوانٍ
  useEffect(() => {
    if (message) {
  ...
  // ✅ دالة فتح نموذج تعديل جراج
  const handleEditGarage = (garage) => {
    console.log('Editing garage:', garage);
    // ✅ إعادة تعيين قيم التعديل
    setEditWorkingDays([]);
    setEditOpeningTime('08:00');
    setEditClosingTime('18:00');
    let servicesStr = '';
    try {
      servicesStr = garage.services ? JSON.parse(garage.services).join(', ') : '';
    } catch (e) {
      servicesStr = garage.services || '';
    }
    setEditingGarage({
      ...garage,
  ...
  const handleDeleteGarage = async (id) => {
    ...
    try {
      ...
      if (res.ok) {
        setMessage('✅ Garage supprimé avec succès !')
        await loadAdminData()
      } else {
        const data = await res.json()
  ...
  // ✅ بدء مؤقت التشخيص
  const handleStartDiagnostic = async (repairId) => {
    const token = localStorage.getItem('admin_token')
    const hours = prompt('⏱️ Durée du diagnostic (en heures):', '48');
    if (!hours || isNaN(hours)) return;
    
    try {
      const res = await fetch(`/api/admin/repairs/${repairId}/start-diagnostic`, {
        method: 'POST',
        headers: { 
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({ duration_hours: parseInt(hours) })
      })
      if (res.ok) {
        setMessage('✅ Diagnostic démarré avec timer ' + hours + 'h')
        loadAdminData()
      }
    } catch (err) {
      setMessage('❌ Erreur')
  ...
  // ✅ بدء مؤقت التنفيذ
  const handleStartProgress = async (repairId) => {
    const token = localStorage.getItem('admin_token')
    const hours = prompt('⏱️ Durée des travaux (en heures):', '72');
    if (!hours || isNaN(hours)) return;
    
    try {
      const res = await fetch(`/api/admin/repairs/${repairId}/start-progress`, {
        method: 'POST',
        headers: { 
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (res.ok) {
        setMessage('✅ Travaux démarrés avec timer ' + hours + 'h')
        loadAdminData()
      }
    } catch (err) {
      setMessage('❌ Erreur')
  ...
  // ✅ إعادة تعيين المؤقت
  const handleResetTimer = async (repairId, timerType) => {
    const token = localStorage.getItem('admin_token')
    if (!window.confirm(`🔄 Réinitialiser le timer ${timerType === 'diagnostic' ? 'de diagnostic' : 'des travaux'} ?`)) return
    
    try {
      const res = await fetch(`/api/admin/repairs/${repairId}/reset-timer`, {
        method: 'POST',
        headers: { 
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      <main className="about-main">
        <section className="diagnostic-section">
          ...
          <div className="cards-grid" style={{gridTemplateColumns: 'repeat(3, 1fr)'}}>
            <div className="stats-card">
              <h3 style={{color: 'red'}}>{stats.totalVehicles || 0}</h3>
              <p>Véhicules enregistrés</p>
            </div>
            <div className="stats-card">
              <h3 style={{color: 'red'}}>{stats.activeDiagnostics || 0}</h3>
              <p>Défauts actifs</p>
            </div>
            <div className="stats-card">
            ...
            <div className="stats-card">
              <h3 style={{color: 'red'}}>{stats.totalUsers || 0}</h3>
              <p>Utilisateurs</p>
            </div>
            <div className="stats-card">
              <h3 style={{color: 'red'}}>{stats.totalGarages || 0}</h3>
              <p>Garages enregistrés</p>
            </div>
            <div className="stats-card">
              <h3 style={{color: 'red'}}>{stats.totalServices || 0}</h3>
              <p>Services à domicile</p>
            </div>
            <div className="stats-card">
              <h3 style={{color: 'red'}}>{stats.activeRepairs || 0}</h3>
              <p>Réparations en cours</p>
            </div>
            <div className="stats-card">
              <h3 style={{color: 'red'}}>{stats.activeAppointments || 0}</h3>
              <p>Rendez-vous en cours</p>
            </div>
            <div className="stats-card">
              <h3 style={{color: 'red'}}>{stats.totalAppointments || 0}</h3>
        ...
        {activeTab === 'garages' && (
          <section className="about-section">
            <div className="about-card">
                ...
                {garages.map(garage => (
                  <div key={garage.id} className="admin-item-card" style={{
                    ...
                    <div style={{flex: '1 1 200px', minWidth: '200px'}}>
                      ...
                      {garage.verified === 0 ? (
                        <span style={{background: '#dc2626', color: 'white', padding: '2px 8px', borderRadius: '4px', fontSize: '0.75rem', fontWeight: 'bold'}}>⏳ Non vérifié | Google Maps</span>
                      ) : (
                    ...
                    <div style={{display: 'flex', gap: '8px', flexWrap: 'wrap', justifyContent: 'flex-end'}}>
                      <button onClick={() => handleEditGarage(garage)} style={{background: '#dc2626', border: 'none', color: 'white', padding: '8px 16px', borderRadius: '8px', cursor: 'pointer', fontSize: '0.85rem', whiteSpace: 'nowrap'}}>✏️ Modifier</button>
                      {garage.verified === 0 && (
              ...
              {/* ✅ نموذج تعديل جراج */}
              {editingGarage && (
                <div style={{background: '#1e293b', padding: '20px', borderRadius: '12px', marginTop: '20px', border: '2px solid #E31837'}}>
                  ...
                  <div style={{display: 'grid', gap: '12px'}}>
                    ...
                    <input placeholder="Ville / Localisation" value={editingGarage.location} onChange={e => setEditingGarage({...editingGarage, location: e.target.value})} className="form-input" />
                    ...
                    <input placeholder="Adresse complète" value={editingGarage.address || ''} onChange={e => setEditingGarage({...editingGarage, address: e.target.value})} className="form-input" />
                    <div className="form-group">
                      <label>Horaires de travail</label>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '15px', flexWrap: 'wrap' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <div style={{display: 'flex', gap: '20px', marginTop: '8px'}}>
                        <label style={{display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer'}}>
                          <input 
                            ...
                            onChange={() => setEditingGarage({...editingGarage, verified: 1})}
                          />
                          <span style={{background: '#64748b', color: 'white', padding: '2px 8px', borderRadius: '4px', fontSize: '0.8rem', fontWeight: 'bold'}}>✔ vérifié | partner MechanicSmart</span>
                        </label>
                        <label style={{display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer'}}>
                          <input 
                            ...
                            name="editGarageStatus" 
                            checked={editingGarage.verified === 0} 
                            onChange={() => setEditingGarage({...editingGarage, verified: 0})}
                          />
                          <span style={{background: '#dc2626', color: 'white', padding: '2px 8px', borderRadius: '4px', fontSize: '0.8rem', fontWeight: 'bold'}}>⏳ Non vérifié | Google Maps</span>
                        </label>
                      </div>
                    ...
                    <div style={{display: 'flex', gap: '10px'}}>
                      <button className="btn btn-primary" onClick={handleUpdateGarage} style={{flex: 1, background: '#16a34a'}}>💾 Enregistrer</button>
                      <button className="btn btn-secondary" onClick={() => setEditingGarage(null)}>❌ Annuler</button>
                    </div>
                  </div>
                </div>
              )}
              ...
              {editingService && (
                <div style={{background: '#1e293b', padding: '20px', borderRadius: '12px', marginTop: '20px', border: '2px solid #E31837'}}>
                  ...
                  <div style={{display: 'grid', gap: '12px'}}>
                      ...
                      <div style={{display: 'flex', gap: '20px', marginTop: '8px'}}>
                        <label style={{display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer'}}>
                          <input 
                            ...
                            onChange={() => setEditingService({...editingService, verified: 1})}
                          />
                          <span style={{background: '#64748b', color: 'white', padding: '2px 8px', borderRadius: '4px', fontSize: '0.8rem', fontWeight: 'bold'}}>✔ vérifié | partner MechanicSmart</span>
                        </label>
                        <label style={{display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer'}}>
                          <input 
                            ...
                            name="editServiceStatus" 
                            checked={editingService.verified === 0} 
                            onChange={() => setEditingService({...editingService, verified: 0})}
                          />
                          <span style={{background: '#dc2626', color: 'white', padding: '2px 8px', borderRadius: '4px', fontSize: '0.8rem', fontWeight: 'bold'}}>⏳ Non vérifié | Google Maps</span>
                        </label>
                      </div>
                    ...
                    <div style={{display: 'flex', gap: '10px'}}>
                      <button className="btn btn-primary" onClick={handleUpdateService} style={{flex: 1, background: '#16a34a'}}>💾 Enregistrer</button>
                      <button className="btn btn-secondary" onClick={() => setEditingService(null)}>❌ Annuler</button>
                    </div>
                  </div>
                </div>
              )}
                ...
                {users.map(user => (
                  <div key={user.id} style={{
                    ...
                    <div>
                      ...
                      <p style={{color: '#94a3b8', fontSize: '0.85rem', margin: '3px 0'}}>
                        🔑 <span style={{color: '#f59e0b', fontWeight: 'bold'}}>••••••••</span> | 🚗 {user.vehicles_count || 0} véhicule(s) | {user.role}
                      </p>
                    </div>
                    {user.role !== 'admin' && (
                  ...
                  <div key={repair.id} style={{
                    ...
                    <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px'}}>
                      <div>
                        ...
                        {/* 🕐 Admin Timer Display */}
                        {repair.status === 'diagnosing' && repair.diagnostic_started_at && (
                          <div style={{marginTop: '8px', display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap'}}>
                            <span style={{color: '#94a3b8', fontSize: '0.8rem'}}>🔍 Timer diagnostic:</span>
                            <TimerDisplay startTime={repair.diagnostic_started_at} maxHours={repair.diagnostic_duration_hours || 48} />
                            <button onClick={() => handleResetTimer(repair.id, 'diagnostic')} style={{background: 'transparent', border: '1px solid #64748b', color: '#94a3b8', padding: '2px 8px', borderRadius: '4px', cursor: 'pointer', fontSize: '0.7rem'}}>🔄 Reset</button>
                          </div>
                        )}
                        {repair.status === 'in_progress' && repair.progress_started_at && (
                          <div style={{marginTop: '8px', display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap'}}>
                            <span style={{color: '#94a3b8', fontSize: '0.8rem'}}>🔧 Timer travaux:</span>
                            <TimerDisplay startTime={repair.progress_started_at} maxHours={repair.progress_duration_hours || 72} />
                            <button onClick={() => handleResetTimer(repair.id, 'progress')} style={{background: 'transparent', border: '1px solid #64748b', color: '#94a3b8', padding: '2px 8px', borderRadius: '4px', cursor: 'pointer', fontSize: '0.7rem'}}>🔄 Reset</button>
                          </div>
                        )}
                      </div>
                      <div style={{textAlign: 'right'}}>
                        ...
                        <div style={{display: 'flex', gap: '5px', flexWrap: 'wrap'}}>
                          {repair.status === 'pending' && (
                            <button className="btn btn-secondary btn-compact" onClick={() => handleStartDiagnostic(repair.id)} style={{fontSize: '0.7rem'}}>🔍 Démarrer Diagnostic</button>
                          )}
                          {repair.status === 'diagnosing' && (
                            <button className="btn btn-secondary btn-compact" onClick={() => handleStartProgress(repair.id)} style={{fontSize: '0.7rem'}}>🔧 Démarrer Travaux</button>
                          )}
                  ...
                  <div key={appt.id} style={{
                    ...
                    <div>
                      ...
                      {appt.status === 'completed' && appt.completed_at && (
                        <p style={{color: '#22c55e', fontSize: '0.8rem', marginTop: '3px'}}>
                          ✅ Terminé le {appt.completed_at}
                        </p>
                      )}
                    </div>
                    <span style={{
                      padding: '3px 10px', borderRadius: '12px',
                      background: appt.status === 'confirmed' ? '#22c55e' : 
                                 appt.status === 'completed' ? '#64748b' :
                                 appt.status === 'cancelled' ? '#dc2626' : '#f59e0b',
...
// =======
// 🚗 Mes Véhicules (CRUD) - Updated with new fields
// =======
function MyVehiclesPage() {
  ...
  const fuelTypes = ['Essence', 'Diesel', 'GPL', 'Électrique', 'Hybride']
  ...
  const loadTechnicalSpecs = async (vehicleId) => {
    try {
      const res = await fetch(`/api/vehicles/${vehicleId}/specs`, {
        headers: { 'Authorization': `Bearer ${userToken}` }
      })
      const data = await res.json()
      setTechnicalSpecs(data.specs)
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.name || !form.year || !form.brand || !form.model) {
      setMessage('❌ Nom, marque, modèle et année sont obligatoires')
      return
    }
    setMessage('')
    ...
    try {
      ...
      if (res.ok) {
        ...
        setShowForm(false)
        setEditing(null)
        setForm({ name: '', year: '', plate_number: '', brand: '', model: '', variant: '', fuel_type: '', transmission: '' })
        loadVehicles()
      } else {
        setMessage(`❌ ${data.error}`)
      }
  ...
  const handleViewSpecs = (vehicleId) => {
    loadTechnicalSpecs(vehicleId)
  }
  ...
  return (
    <div className="page-shell">
      <AppHeader subTitle="MES VÉHICULES" />

      <section className="about-section">
        <div className="about-card" style={{maxWidth: '700px'}}>
          <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px'}}>
            <h2>🚗 Mes véhicules ({vehicles.length})</h2>
            <button className="btn btn-primary" onClick={() => { setShowForm(!showForm); setEditing(null); setForm({ name: '', year: '', plate_number: '', brand: '', model: '', variant: '', fuel_type: '', transmission: '' }); setMessage(''); setTechnicalSpecs(null) }}>
              {showForm ? '❌ Annuler' : '➕ Ajouter'}
            </button>
          </div>

          {message && <div className="message" style={{marginBottom: '15px'}}>{message}</div>}

          {showForm && (
            <div style={{background: '#1e293b', padding: '20px', borderRadius: '12px', marginBottom: '20px'}}>
              <h3 style={{color: 'white', marginBottom: '15px'}}>{editing ? '✏️ Modifier le véhicule' : '➕ Nouveau véhicule'}</h3>
              <form onSubmit={handleSubmit} style={{display: 'grid', gap: '12px'}}>
                ...
                <input
                  placeholder="Variante (optionnel, ex: IV, V, Phase 2)"
                  value={form.variant}
                  onChange={e => setForm({...form, variant: e.target.value})}
                  className="form-input"
                />
                <input
                  placeholder="Nom du véhicule (ex: Renault Clio 2023)"
                  value={form.name}
          ...
          {technicalSpecs && (
            <div style={{background: '#0f172a', padding: '16px', borderRadius: '12px', border: '1px solid #E31837', marginBottom: '20px'}}>
              <h3 style={{color: '#E31837', marginBottom: '12px'}}>🔧 Spécifications techniques</h3>
              <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px'}}>
                <div><span style={{color: '#94a3b8'}}>Moteur:</span> <span style={{color: 'white'}}>{technicalSpecs.engine_code || '-'}</span></div>
                <div><span style={{color: '#94a3b8'}}>Cylindrée:</span> <span style={{color: 'white'}}>{technicalSpecs.engine_size ? `${technicalSpecs.engine_size}L` : '-'}</span></div>
                <div><span style={{color: '#94a3b8'}}>Puissance:</span> <span style={{color: 'white'}}>{technicalSpecs.horsepower ? `${technicalSpecs.horsepower} CH` : '-'}</span></div>
                <div><span style={{color: '#94a3b8'}}>Couple:</span> <span style={{color: 'white'}}>{technicalSpecs.torque ? `${technicalSpecs.torque} Nm` : '-'}</span></div>
                <div><span style={{color: '#94a3b8'}}>ABS:</span> <span style={{color: technicalSpecs.abs ? '#22c55e' : '#dc2626'}}>{technicalSpecs.abs ? '✅ Oui' : '❌ Non'}</span></div>
                <div><span style={{color: '#94a3b8'}}>ESP:</span> <span style={{color: technicalSpecs.esp ? '#22c55e' : '#dc2626'}}>{technicalSpecs.esp ? '✅ Oui' : '❌ Non'}</span></div>
                <div><span style={{color: '#94a3b8'}}>Alimentation:</span> <span style={{color: 'white'}}>{technicalSpecs.fuel_system || '-'}</span></div>
                <div><span style={{color: '#94a3b8'}}>ECU:</span> <span style={{color: 'white'}}>{technicalSpecs.ecu_type || '-'}</span></div>
                <div><span style={{color: '#94a3b8'}}>Protocole OBD:</span> <span style={{color: 'white'}}>{technicalSpecs.obd_protocol || '-'}</span></div>
              </div>
            </div>
          )}
          ...
          {loading ? (
            ...
            <div style={{textAlign: 'center', padding: '40px 0'}}>
              <p style={{color: '#94a3b8', marginBottom: '20px'}}>Aucun véhicule enregistré.</p>
            ...
            <div style={{display: 'grid', gap: '12px'}}>
              {vehicles.map(v => (
                <div key={v.id} style={{
                  ...
                  <div style={{flex: 1}}>
                    <strong style={{color: 'white', fontSize: '1.05rem'}}>{v.name}</strong>
                    <span style={{color: '#94a3b8', marginLeft: '8px'}}>({v.year})</span>
                    <p style={{color: '#64748b', fontSize: '0.85rem', margin: '3px 0'}}>
                      🚗 {v.brand ? `${v.brand} ${v.model}` : ''} {v.variant ? `(${v.variant})` : ''}
                      {v.fuel_type ? ` | ⛽ ${v.fuel_type}` : ''}
                      {v.transmission ? ` | ⚙️ ${v.transmission}` : ''}
                    </p>
                    <p style={{color: '#64748b', fontSize: '0.85rem', margin: '3px 0'}}>
                      {v.plate_number ? `🔖 ${v.plate_number} | ` : ''}
                      État: {v.status === 'ok' ? '✅ Bon' : v.status === 'warning' ? '⚠️ Attention' : '🔴 Critique'}
                      {v.total_repairs_count > 0 && ` | 🔧 ${v.total_repairs_count} réparation(s)`}
                    </p>
                    {v.last_repair_date && (
                      <p style={{color: '#64748b', fontSize: '0.8rem'}}>
                        Dernière réparation: {v.last_repair_date}
                      </p>
                    )}
                    {v.technical_specs && (
                      <p style={{color: '#3b82f6', fontSize: '0.8rem', marginTop: '4px'}}>
                        🔧 {v.technical_specs.engine_code && `${v.technical_specs.engine_code} - `}
                        {v.technical_specs.horsepower ? `${v.technical_specs.horsepower} CH` : 'Spécifications auto-générées'}
                      </p>
                    )}
                  </div>
                  <div style={{display: 'flex', gap: '8px'}}>
                    <button className="btn btn-secondary btn-compact" onClick={() => handleViewSpecs(v.id)}>🔍</button>
                    <button className="btn btn-secondary btn-compact" onClick={() => handleEdit(v)}>✏️</button>
                    <button className="btn btn-primary btn-compact" onClick={() => handleDelete(v.id)} style={{background: '#dc2626'}}>🗑️</button>
                  </div>
                </div>
              ))}
            </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/garages?${cacheBuster}`, { cache: 'no-store' })
      .then((res) => res.json())
      .then((data) => {
        console.log('🔍 [App] Garages reçus:', data);
        setGarages(data.garages || [])
      })
      .catch((err) => console.error('❌ [App] Erreur chargement garages:', err))

    fetch(`/api/services?${cacheBuster}`, { cache: 'no-store' })
      .then((res) => res.json())
      .then((data) => {
        console.log('🔍 [App] Services reçus:', data);
        setServices(data.services || [])
      })
      .catch((err) => console.error('❌ [App] Erreur chargement services:', err))

    fetch(`/api/public/stats?${cacheBuster}`, { cache: 'no-store' })
      .then((res) => res.json())
      .then((data) => {
        console.log('🔍 [App] Stats reçues:', data);
        setStats(data)
      })
      .catch((err) => {
        console.error('❌ [App] Erreur chargement stats:', err);
        setStats({})
      })
  }, [])
