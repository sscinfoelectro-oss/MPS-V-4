              ...
              return (
                <div key={garage.id} className="garage-card">
                  ...
                  <div className="card-actions">
                    ...
                    {userToken && (
                      <button className="btn btn-primary btn-compact" style={{background: '#16a34a'}} onClick={() => setShowBooking(showBooking === garage.id ? null : garage.id)}>
                        📅 Réserver
                      </button>
                    )}
              ...
              {showGarageForm && (
                <div style={{background: '#1e293b', padding: '20px', borderRadius: '12px', marginBottom: '20px'}}>
                  ...
                  <div style={{display: 'grid', gap: '10px'}}>
                    <input placeholder="Nom" value={newGarage.name} onChange={e => setNewGarage({...newGarage, name: e.target.value})} className="form-input" />
                    <input placeholder="Ville" value={newGarage.location} onChange={e => setNewGarage({...newGarage, location: e.target.value})} className="form-input" />
                    <input placeholder="Téléphone" value={newGarage.phone} onChange={e => setNewGarage({...newGarage, phone: e.target.value})} className="form-input" />
                    ...
                    <button className="btn btn-primary" onClick={handleAddGarage}>✅ Ajouter</button>
                  </div>
                </div>
              )}
                ...
                {garages.map(garage => (
                  <div key={garage.id} className="admin-item-card" style={{
                    ...
                    <div>
                      ...
                      <p style={{color: '#94a3b8', fontSize: '0.85rem', margin: '3px 0'}}>📍 {garage.location} ⭐ {garage.rating}</p>
                    </div>
                    <div style={{display: 'flex', gap: '8px'}}>
                      <button onClick={() => handleDeleteGarage(garage.id)} style={{background: '#dc2626', border: 'none', color: 'white', padding: '6px 12px', borderRadius: '6px', cursor: 'pointer'}}>🗑️</button>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {activeTab === 'services' && (
          <section className="about-section">
            <div className="about-card">
              ...
              {showServiceForm && (
                <div style={{background: '#1e293b', padding: '20px', borderRadius: '12px', marginBottom: '20px'}}>
                  <h3 style={{color: 'white', marginBottom: '15px'}}>Nouveau Service</h3>
                  <div style={{display: 'grid', gap: '10px'}}>
                    <input placeholder="Nom" value={newService.name} onChange={e => setNewService({...newService, name: e.target.value})} className="form-input" />
                    ...
                    <input placeholder="Téléphone" value={newService.phone} onChange={e => setNewService({...newService, phone: e.target.value})} className="form-input" />
                    <input placeholder="Services (séparés par virgule)" value={newService.services_list} onChange={e => setNewService({...newService, services_list: e.target.value})} className="form-input" />
                    <button className="btn btn-primary" onClick={handleAddService}>✅ Ajouter</button>
                  </div>
                </div>
              )}
              <div style={{display: 'grid', gap: '10px'}}>
                {services.map(service => (
                  <div key={service.id} style={{padding: '12px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px'}}>
                    <div>
                      <strong style={{color: 'white'}}>{service.name}</strong>
                      <p style={{color: '#94a3b8', fontSize: '0.85rem', margin: '3px 0'}}>📍 {service.location} ⭐ {service.rating}</p>
                    </div>
                    <button onClick={() => handleDeleteService(service.id)} style={{background: '#dc2626', border: 'none', color: 'white', padding: '6px 12px', borderRadius: '6px', cursor: 'pointer'}}>🗑️</button>
                  </div>
                ))}
              </div>
            </div>
