                ...
                {services.map(service => (
                  <div key={service.id} style={{
                    ...
                    <div style={{flex: '1 1 200px', minWidth: '200px'}}>
                      ...
                      {service.verified === 0 ? (
                        <span style={{background: '#f59e0b', color: '#000', padding: '2px 8px', borderRadius: '4px', fontSize: '0.75rem', fontWeight: 'bold'}}>⏳ Non vérifié | {service.source}</span>
                      ) : (
                        <span style={{background: '#22c55e', color: '#000', padding: '2px 8px', borderRadius: '4px', fontSize: '0.75rem', fontWeight: 'bold'}}>✔ vérifié | partner MechanicSmart</span>
                      )}
                    </div>
                    <div style={{display: 'flex', gap: '8px', flexWrap: 'wrap', justifyContent: 'flex-end'}}>
                      <button onClick={() => { setEditingService({ ...service, services_list: JSON.parse(service.services_list || '[]').join(', ') }) }} style={{background: '#2563eb', border: 'none', color: 'white', padding: '8px 16px', borderRadius: '8px', cursor: 'pointer', fontSize: '0.85rem', whiteSpace: 'nowrap'}}>✏️ Modifier</button>
