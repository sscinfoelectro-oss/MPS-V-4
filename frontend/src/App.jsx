...
function MyVehiclesPage() {
  ...
  const [form, setForm] = useState({ name: '', year: '', plate_number: '' })
  const userToken = localStorage.getItem('user_token')
  const navigate = useNavigate()
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.name || !form.year) {
      setMessage('❌ Nom et année sont obligatoires')
      return
    }
    setMessage('')
    ...
    try {
      ...
      if (res.ok) {
        ...
        setShowForm(false)
        setEditing(null)
        setForm({ name: '', year: '', plate_number: '' })
        loadVehicles()
      } else {
        setMessage(`❌ ${data.error}`)
      }
  ...
  const handleEdit = (vehicle) => {
    ...
    setForm({ name: vehicle.name, year: String(vehicle.year), plate_number: vehicle.plate_number || '' })
    setShowForm(true)
    setMessage('')
  }
      ...
      <section className="about-section">
        <div className="about-card" style={{maxWidth: '700px'}}>
          <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px'}}>
            ...
            <button className="btn btn-primary" onClick={() => { setShowForm(!showForm); setEditing(null); setForm({ name: '', year: '', plate_number: '' }); setMessage('') }}>
          ...
          {showForm && (
            <div style={{background: '#1e293b', padding: '20px', borderRadius: '12px', marginBottom: '20px'}}>
              ...
              <form onSubmit={handleSubmit} style={{display: 'grid', gap: '12px'}}>
                <input
                  placeholder="Nom du véhicule (ex: Renault Clio)"
                  value={form.name}
                ...
                <div key={v.id} style={{
                  ...
                  <div style={{flex: 1}}>
                    ...
                    <p style={{color: '#64748b', fontSize: '0.85rem', margin: '3px 0'}}>
                      🚗 {v.plate_number ? `Plaque: ${v.plate_number} | ` : ''}
